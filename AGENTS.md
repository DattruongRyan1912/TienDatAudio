...
- Ghi baseline, rủi ro và giả định đã được xác minh vào mục mới trong `docs/WORKLOG.md` đối với công việc có ảnh hưởng đáng kể.
...
- Sau mỗi công việc đáng kể, append một entry vào `docs/WORKLOG.md`; không viết lại lịch sử. Nếu cần sửa, thêm correction entry mới.
...
Chi tiết mutation matrix, evidence và rollback policy nằm tại `docs/AGENT_GOVERNANCE.md`. Quy trình vận hành production nằm tại `docs/DEPLOYMENT_RUNBOOK.md`.
