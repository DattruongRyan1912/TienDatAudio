...
export default function AdminTaxonomyManager({ kind }: { kind: 'categories' | 'brands' }) {
  ...
  function startEdit(item: RecordItem) { setEditingId(item.id); setName(item.name); setDescription(item.description || ''); setMessage('') }
  async function save(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault()
    setSaving(true)
    setMessage('')
    const isEditing = Boolean(editingId)
    const response = await fetch(`/api/admin/${kind}`, { method: isEditing ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, description, ...(editingId ? { id: editingId } : {}) }) })
