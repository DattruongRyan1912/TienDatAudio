  ...
  return <div className="sonic-page article-page pt-28 md:pt-36">
    ...
    <article className={`article-container pb-20 md:pb-28 ${preview ? 'pt-12' : ''}`}>
      ...
      <div className={`article-content-grid ${headings.length > 1 ? '' : 'article-content-grid--without-toc'}`}>
        {headings.length > 1 && <div className="article-toc-reveal"><aside className="article-toc"><p className="sonic-label">Trong bài viết</p><nav className="mt-4 grid gap-2 border-l border-white/10 pl-4">{headings.map((heading) => <a key={heading.id} href={`#${heading.id}`} className={`text-xs leading-5 text-[#858989] hover:text-[#d4af37] ${heading.depth === 3 ? 'pl-3' : ''}`}>{heading.text}</a>)}</nav></aside></div>}
