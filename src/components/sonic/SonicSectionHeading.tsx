...
export default function SonicSectionHeading({ label, title, copy, href, linkLabel = 'Xem tất cả' }: { label: string; title: string; copy?: string; href?: string; linkLabel?: string }) {
  return (
    <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
      ...
      {href && <Link href={href} className="sonic-section-link group inline-flex shrink-0 items-center gap-2 text-xs font-bold uppercase tracking-[0.14em]">{linkLabel}<ArrowUpRight size={15} className="transition-transform duration-200 group-hover:translate-x-1 group-hover:-translate-y-1" /></Link>}
    </div>
  )
}
