...
export default function SonicHeader() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 md:px-6 md:pt-5">
      <div className="sonic-glass mx-auto flex max-w-[1440px] items-center justify-between px-4 py-3 md:px-6">
        ...
        <nav className="hidden items-center gap-5 lg:flex" aria-label="Điều hướng chính">
          {navigation.map((item) => (
            <Link key={item.href} href={item.href} className={`text-[0.64rem] font-bold uppercase tracking-[0.14em] transition-colors hover:text-[#d4af37] ${pathname === item.href ? 'text-[#d4af37]' : 'text-[#a7aaaa]'}`}>
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/contact" className="sonic-button sonic-button-gold hidden min-h-9 px-4 text-[0.62rem] sm:inline-flex">Nhận tư vấn</Link>
        ...
        {searchOpen && (
          <form onSubmit={submitSearch} className="absolute inset-x-3 top-[calc(100%+8px)] flex gap-2 border border-white/10 bg-[#0d0d0d] p-2 md:inset-x-auto md:right-0 md:w-[360px]">
      ...
      {menuOpen && (
        <div className="sonic-glass mx-auto mt-2 max-w-[1440px] p-4 lg:hidden">
