...
export default async function KnowledgePage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <div className="sonic-page pt-28 md:pt-36">
    ...
    {!featured ? <section className="border-y border-white/10 bg-[#0d0d0d] py-20"><div className="sonic-container"><p className="text-sm text-[#858989]">Không tìm thấy bài viết phù hợp với “{params.q}”.</p><Link href="/kien-thuc" className="sonic-button sonic-button-ghost mt-6">Xóa bộ lọc</Link></div></section> : <>
      ...
      <section className="sonic-container py-16 md:py-24"><SonicReveal><div className="flex items-end justify-between"><div><p className="sonic-label">Archive / Latest notes</p><h2 className="sonic-title mt-4">Góc nhìn mới nhất.</h2></div><span className="sonic-label hidden text-[#858989] sm:block">{rest.length.toString().padStart(2, '0')} bài viết</span></div></SonicReveal><div className="mt-12 grid gap-x-8 gap-y-12 md:grid-cols-2">{rest.map((post, index) => <SonicReveal key={post.id} delay={Math.min(index * 0.06, 0.24)}><Link href={`/kien-thuc/${post.slug}`} className="group grid gap-5 border-t border-white/15 pt-5 sm:grid-cols-[.85fr_1.15fr]"><div className="relative aspect-[1.15] overflow-hidden bg-[#111111]"><Image src={post.featuredImage || '/images/sonic-hero.png'} alt={post.title} fill sizes="(min-width: 768px) 30vw, 100vw" className="sonic-image-muted object-cover" /></div><div><div className="flex items-center justify-between"><span className="sonic-label text-[#858989]">{String(index + 2).padStart(2, '0')} / {post.category}</span><ArrowUpRight size={15} className="text-[#858989] transition-colors group-hover:text-[#d4af37]" /></div><h3 className="mt-5 text-xl font-bold leading-tight tracking-[-0.03em] text-[#e5e2e1] transition-colors group-hover:text-[#d4af37]">{post.title}</h3><p className="mt-3 text-sm leading-6 text-[#9ea2a2]">{post.excerpt}</p><p className="mt-5 text-[0.62rem] font-bold uppercase tracking-[0.15em] text-[#707474]">{post.readingTime} phút đọc · {post.author}</p></div></Link></SonicReveal>)}</div></section>
    </>}
  </div>
}
